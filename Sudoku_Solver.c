...
int valid_row(int sudoku[9][9], int row, int num){
    int counter =0; 
    for (int j=0; j<9; j++){
        if (sudoku[row][j]==num){
            counter++;
...
int valid_col(int sudoku[9][9], int col, int num){
    int counter =0; 
    for (int i=0; i<9; i++){
        if (sudoku[i][col]==num){
            counter++;
...
int valid_square(int sudoku[9][9], int startrow, int startcol, int num){
  int row=0;
  int col=0;
...
int check_sudoku(int sudoku[9][9])
{
   ...
   * *****/
   ...
   for (int i=0; i<9;i++){
        for (int j=0; j<9; j++){
            if (sudoku[i][j] !=0) {
                if ((valid_square(sudoku, i, j, sudoku[i][j]) > 1) || (valid_col(sudoku, j, sudoku[i][j])>1) || (valid_row(sudoku, i, sudoku[i][j])>1)){
                    return 1;
        }
        }
     }
...
int solved(int sudoku[9][9])
{
  ...
  for (int i=0; i<9; i++){
    for (int j=0; j<9; j++){
      if (sudoku[i][j]==0 || (valid_square(sudoku, i, j, sudoku[i][j]) != 1) || (valid_col(sudoku, j, sudoku[i][j])!=1) || (valid_row(sudoku, i, sudoku[i][j])!=1)){
        return 0;
      }
    }
  }
...
int find_emp_col(int sudoku[9][9]){
    for (int i=0; i<9; i++){
        for (int j=0; j<9; j++){
            if (sudoku[i][j]==0){
                return j; 
...
int find_emp_row(int sudoku[9][9]){
    for (int i=0; i<9; i++){
        for (int j=0; j<9; j++){
            if (sudoku[i][j]==0){
                return i; 
...
void solve_sudoku(int sudoku[9][9], int depth)
{
  ...
  int z_row = find_emp_row(sudoku);
  int z_col = find_emp_col(sudoku);
  

  for (int n = 1; n <= 9; n++)
  {
    // if the number is not already in the grid
    if ((valid_col(sudoku, z_col, n) == 0) && (valid_row(sudoku, z_row, n) == 0) && (valid_square(sudoku, z_row, z_col, n) == 0))
    {
      sudoku[z_row][z_col] = n;
      int next_row = find_emp_row(sudoku);
      int next_col = find_emp_col(sudoku);
